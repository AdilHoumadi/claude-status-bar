...
/// Used to keep automated/headless Claude runs (e.g. the Mnemo server's `claude -p` calls)
/// out of the status bar. Prefixes are read from a plain file, one path per line.
public enum IgnoreList {
    ...
    public static func prefixes(from url: URL) -> [String] {
        ...
        return text
            .split(separator: "\n")
            .map { expandTilde(String($0).trimmingCharacters(in: .whitespaces)) }
            .filter { !$0.isEmpty }
    }
