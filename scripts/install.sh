...
# Seed the ignore list (skip automated/headless runs) if absent.
IGNORE="$HOME/.claude/statusbar/ignore.txt"
mkdir -p "$HOME/.claude/statusbar"
[ -f "$IGNORE" ] || printf '# One project path per line; sessions under these paths are never captured.\n# Use for automated/headless \x60claude -p\x60 runners.\n' > "$IGNORE"
